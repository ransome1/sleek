...
const migrations = {
  '2.0.0': (config) => {
    ...
    config.set('accordionOpenState', [true, true, true, false, false, false, false, false, false])
    config.set('files', [])
    ...
    config.set('customStylesPath', path.join(userDataDirectory, 'customStyles.css'))
