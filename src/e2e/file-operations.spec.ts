...
import { createTodoFile, initTestDir, cleanupTestDir } from "./helpers/fileHelper";
...
test.describe("File Operations - Open & Create (Full Workflow)", () => {
  ...
  test("should load an existing todo file and display todos", async ({
    ...
    const todoFile = createTodoFile("test-todos.txt", todoContent);
    ...
    const splashScreenStillVisible = await splashScreen
      .isVisible()
      .catch(() => false);
    ...
    if (!splashScreenStillVisible) {
      ...
      const gridOrContent = page.locator(".MuiList-root, [role='list'], #grid");
      const isContentVisible = await gridOrContent
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test("should add a file to the app's file list", async ({ page }) => {
    ...
    // Create a file
    const todoFile = createTodoFile("add-me.txt", "First task");

    // Add the file to the app
    await page.evaluate((filePath) => {
      (window as any).api.ipcRenderer.send("addFile", filePath);
    }, todoFile);
    ...
    const files = await page.evaluate(() => {
      return (window as any).api.store.getConfig("files") || [];
    });
  ...
  test("should verify file content persists after loading", async ({
    ...
    const todoFile = createTodoFile("unique-test.txt", uniqueContent);
    ...
    const todosLoaded = await page.evaluate(() => {
      return new Promise<boolean>((resolve) => {
        (window as any).api.ipcRenderer.send("requestData", "");

        const unsubscribe = (window as any).api.ipcRenderer.on(
          "requestData",
          (data: any) => {
            unsubscribe();
            // Check if we got todo data back
            resolve(
              data && data.todoData && Array.isArray(data.todoData) && data.todoData.length > 0
            );
          }
        );
  ...
  test("should handle multiple files in file list", async ({ page }) => {
    ...
    // Create first file
    const file1 = createTodoFile("file1.txt", "File 1 task");
    await page.evaluate((filePath) => {
      (window as any).api.ipcRenderer.send("addFile", filePath);
    }, file1);
    ...
    // Create second file
    const file2 = createTodoFile("file2.txt", "File 2 task");
    await page.evaluate((filePath) => {
      (window as any).api.ipcRenderer.send("addFile", filePath);
    }, file2);
...
Task with multiple +projects and @contexts`;
    ...
    const todoFile = createTodoFile("complex-todos.txt", complexContent);
  ...
  test("should preserve file path after loading", async ({ page }) => {
    ...
    const todoFile = createTodoFile("persistent.txt", "Task");
    ...
    // Get files config
    const files = await page.evaluate(() => {
      const config = (window as any).api.store.getConfig("files");
      return config || [];
    });
...
Task with URLs: https://github.com/ransome1/sleek`;
    ...
    const todoFile = createTodoFile("special-chars.txt", specialContent);
      ...
      return new Promise<boolean>((resolve) => {
        ...
        const unsubscribe = (window as any).api.ipcRenderer.on(
          ...
          (data: any) => {
            unsubscribe();
            resolve(
              data && data.todoData && Array.isArray(data.todoData)
            );
          }
        );
  ...
  test("should update file list in navigation after adding file", async ({
    ...
    const todoFile = createTodoFile("nav-test.txt", "Test task");
    ...
    const isVisible = await addTodoButton.isVisible().catch(() => false);
